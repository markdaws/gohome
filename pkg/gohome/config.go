...
func NewDefaultConfig(systemPath, webUIPath string) *Config {
	...
	cfg := Config{
		SystemPath:     path.Join(systemPath, "gohome.json"),
		EventLogPath:   path.Join(systemPath, "events.json"),
		AutomationPath: path.Join(systemPath, "automation"),
		WebUIPath:      webUIPath,
		WWWAddr:        addr,
		WWWPort:        "8000",
		UPNPNotifyAddr: addr,
